<x-layout title="Accounts">
    <div class="container mx-auto px-6 py-8">
        <div class="flex justify-end mb-8">
            <a href="accounts/create"
                class="inline-flex items-center px-6 py-3 bg-blue-700 font-medium rounded-md hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-200 ease-in-out shadow-sm custom-btn-text">
        ...
        @if (session('success'))
            <div class="bg-green-50 border-l-4 border-green-600 text-green-800 p-4 rounded-lg mb-8 flex items-center justify-between shadow-sm"
                role="alert">
                        ...
                        @foreach ($accounts as $account)
                            <tr class="hover:bg-gray-50 transition duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $account['name'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    <a href="http://{{ $account['website'] }}" target="_blank" rel="noopener noreferrer"
                                        class="text-blue-600 hover:text-blue-800">{{ $account['website'] }}</a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    <a href="tel:{{ $account['phone'] }}"
                                        class="text-blue-600 hover:text-blue-800">{{ $account['phone'] }}</a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $account['industry'] }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <a href="/accounts/{{ $account['id'] }}"
                                            ...
                                            title="View" data-tooltip-target="tooltip-view">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <a href="/accounts/{{ $account['id'] }}/edit"
                                            ...
                                            title="Edit" data-tooltip-target="tooltip-edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <!-- Delete Button -->
                                        <form action="/accounts/{{ $account['id'] }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="text-red-600 hover:text-red-900 p-2 rounded-full hover:bg-red-100 transition-all duration-200"
                                                title="Delete" data-tooltip-target="tooltip-delete"
    ...
    <div id="tooltip-view" role="tooltip"
        class="inline-block absolute invisible z-10 py-1 px-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 transition-opacity duration-300">
        View details
    ...
    <script>
        // This script can remain the same as it is generic.
